...
var cache: { [x: string]: any; } = undefined;
...
async function run() {
    ...
    writeFileSyncRecursive('../../create_holograms/cache.json', JSON.stringify(cache, null, 4));
}
...
async function createHologram(
    ...
    if (cacheData.texture === undefined || (cacheData.text !== undefined && cacheData.text.replace(/Â/g, '') !== text)) {
        ...
        writeFileSyncRecursive(texturePath, Buffer.from(cacheData.texture, 'base64'));
    }
...
function exportTexture(image: Jimp, identifier: string, path: string) {
    ...
    image.write(path, (err, image) => {
        ...
        writeFileSyncRecursive('../../create_holograms/cache.json', Buffer.from(JSON.stringify(cache, null, 4), 'utf-8'))
    });
}
