...
function run() {
    ...
    var textures = fs.readdirSync('data/create_glyphs/textures');
    ...
    var index = 0;
    console.log("Compiling glyphs at ".concat(config.scale, " scale and with image smoothing ").concat(!config.imageSmoothing ? "disabled" : "enabled", "!"));
    var translateKeys = new Map();
    textures.forEach(function (texture) {
        ...
        var letter = String.fromCodePoint(parseInt('0xE1' + (parseInt(config.startingIndex, 16) + index).toString(16), 16));
    ...
    if (config.createTranslateKeys !== undefined && config.createTranslateKeys) {
        glob_1.glob.sync('RP/texts/**/*.lang').forEach(function (langFile) {
            var langData = fs.readFileSync(langFile, 'utf-8');
            translateKeys.forEach(function (value, key) {
                langData += "\nglyph.".concat(key, "=").concat(value);
            });
...
function getConfig() {
    return JSON.parse(fs.readFileSync('data/create_glyphs/config.json', 'utf-8'));
}
run();
